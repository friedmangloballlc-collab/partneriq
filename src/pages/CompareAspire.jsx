...
/* ─────────────────────────────────────────────
   COMPARISON DATA — DealStage vs Aspire
───────────────────────────────────────────── */
const COMPETITOR = "Aspire";
...
const FEATURE_ROWS = [
  { feature: "Free tier",                     dealstage: true,  aspire: false },
  { feature: "AI Match Engine",               dealstage: true,  aspire: "partial" },
  { feature: "Monte Carlo ROI Simulator",     dealstage: true,  aspire: false },
  { feature: "Pitch Competition marketplace", dealstage: true,  aspire: false },
  { feature: "Personal Manager role",         dealstage: true,  aspire: false },
  { feature: "88 platform integrations",      dealstage: true,  aspire: "partial" },
  { feature: "Contract templates",            dealstage: true,  aspire: true },
  { feature: "Talent CRM",                    dealstage: true,  aspire: true },
  { feature: "Campaign analytics",            dealstage: true,  aspire: true },
  { feature: "Enterprise / agency tier",      dealstage: true,  aspire: true },
];

const CHOOSE_COMPETITOR = [
  "You are a mid-market brand with a dedicated social media team already using Aspire",
  "You run gifting-heavy campaigns and need Aspire's product seeding workflows",
  "You want a fully managed service option where the vendor handles execution",
  "Your primary need is Instagram and TikTok creator discovery at scale",
];

const CHOOSE_DEALSTAGE = [
  "You want to start without a $2,000/month commitment",
  "You need AI-powered matching beyond keyword search and category filters",
  "You represent talent or manage talent deals — Aspire has no manager role",
  "You want the Monte Carlo ROI Simulator to justify deal budgets internally",
  "You need broader integration coverage across 88 platforms",
];

/* ─────────────────────────────────────────────
   PAGE
───────────────────────────────────────────── */
export default function CompareAspire() {
  return (
        ...
        <Fade>
          ...
          <div style={{ background: T.bgCard, border: `1px solid ${T.border}`, borderRadius: 16, padding: "36px 32px" }}>
            <p style={{ ...sans, fontSize: 17, color: T.cream, lineHeight: 1.75, marginBottom: 16 }}>
              <strong style={{ color: T.gold }}>Aspire</strong> is a creator marketing platform designed for mid-market and enterprise brands, starting at <strong>$2,000/month</strong>. It has solid creator discovery, gifting workflows, and campaign management tools primarily aimed at brand social teams running ongoing influencer programs.
            </p>
            <p style={{ ...sans, fontSize: 17, color: T.cream, lineHeight: 1.75 }}>
              <strong style={{ color: T.gold }}>DealStage</strong> starts free, includes an AI Match Engine, Monte Carlo ROI Simulator, and the only dedicated <strong>personal manager role</strong> in the market — making it the right tool for brands, talent, agencies, and managers alike, without a $24K annual floor.
            </p>
          </div>
        </Fade>
      </section>
              ...
              <tbody>
                {FEATURE_ROWS.map((row, i) => (
                  <tr key={i} style={{ background: i % 2 === 0 ? "transparent" : "rgba(212,176,78,0.02)" }}>
                    ...
                    <td style={{ textAlign: "center", padding: "14px 18px" }}>
                      {row.aspire === true ? <CheckCircle size={18} style={{ color: T.green }} /> :
                       row.aspire === "partial" ? <Minus size={18} style={{ color: T.amber }} /> :
      ...
      <section style={{ maxWidth: 860, margin: "0 auto", padding: "96px 24px 120px", textAlign: "center" }}>
        <Fade>
          ...
          <h2 style={{ ...serif, fontSize: "clamp(32px, 5vw, 56px)", fontWeight: 700, color: T.cream, marginBottom: 20 }}>
            Try DealStage free — no $2K/month required
          </h2>
