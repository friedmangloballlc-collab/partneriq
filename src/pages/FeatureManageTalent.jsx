...
const CAPABILITIES = [
  {
    ...
    desc: "Edit your talent's profile, bio, and media kit on their behalf. Update rate cards, availability, and connected platforms — keep everything polished and current.",
  },
  {
    Icon: Handshake,
    title: "Deal Negotiation",
    desc: "Respond to brands and negotiate on behalf of your talent. Review incoming proposals, send counter-offers, and manage the entire back-and-forth without forwarding emails.",
  },
  {
    Icon: TrendingUp,
    title: "Revenue Dashboard",
    desc: "Track all earnings with automatic commission calculation. See total revenue, your cut, and your talent's payout broken down by deal, brand, and time period.",
  },
  {
    Icon: Bell,
    title: "Opportunity Alerts",
    desc: "Get notified instantly when brands want your talent. Never miss an inbound interest, never leave money on the table — every opportunity lands in your dashboard first.",
  },
];
...
───────────────────────────────────────────── */
const ACTIVE_DEALS = [
  { brand: "Nike", value: "$85K", stage: "Active", daysLeft: 12 },
  { brand: "Spotify", value: "$12.5K", stage: "Negotiating", daysLeft: 5 },
  { brand: "Sephora", value: "$34K", stage: "Contracted", daysLeft: 30 },
  { brand: "Apple", value: "$220K", stage: "Proposed", daysLeft: 3 },
  { brand: "Red Bull", value: "$45K", stage: "Active", daysLeft: 8 },
];

const NOTIFICATIONS = [
  { brand: "Adidas", message: "Wants to partner for summer campaign", time: "2m ago", hot: true },
  { brand: "Amazon", message: "Responded to your counter-offer", time: "18m ago", hot: false },
  { brand: "Samsung", message: "New inbound deal proposal — $95K", time: "1h ago", hot: true },
];

const REVENUE_BARS = [55, 70, 45, 88, 72, 100];
const REVENUE_MONTHS = ["Oct", "Nov", "Dec", "Jan", "Feb", "Mar"];
  ...
  return (
    <div style={{ background: T.bgCard, border: `1px solid ${T.border}`, borderRadius: 16, overflow: "hidden", boxShadow: "0 32px 80px rgba(0,0,0,0.6)" }}>
      ...
      <div style={{ background: "#0a0909", borderBottom: `1px solid ${T.borderAlt}`, padding: "12px 20px", display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <span style={{ ...mono, fontSize: 11, color: T.creamDim, marginLeft: 12, opacity: 0.6 }}>dealstage.app / manage / talent</span>
      </div>

      <div style={{ padding: "24px", display: "grid", gridTemplateColumns: "1fr 300px", gap: 20 }}>
        {/* Left Column */}
        <div>
          {/* Talent Profile Card */}
          <div style={{ background: T.bgCardAlt, border: `1px solid ${T.border}`, borderRadius: 12, padding: "16px 18px", marginBottom: 16, display: "flex", gap: 14, alignItems: "center" }}>
            <div style={{ width: 52, height: 52, borderRadius: "50%", background: `linear-gradient(135deg, ${T.goldDim}, ${T.amber})`, display: "flex", alignItems: "center", justifyContent: "center", ...sans, fontSize: 20, fontWeight: 700, color: T.bg, flexShrink: 0 }}>J</div>
            <div style={{ flex: 1 }}>
              <div style={{ ...sans, fontSize: 15, fontWeight: 600, color: T.cream, marginBottom: 2 }}>Jordan Reeves</div>
              <div style={{ ...mono, fontSize: 11, color: T.creamDim }}>Athlete · 4.2M followers · 5.1% engagement</div>
            </div>
            <div style={{ textAlign: "right" }}>
              <div style={{ ...mono, fontSize: 10, color: T.creamDim, marginBottom: 4 }}>YTD REVENUE</div>
              <div style={{ ...serif, fontSize: 20, fontWeight: 700, color: T.gold }}>$340K</div>
            </div>
          </div>

          {/* Active Deals */}
          <div style={{ ...mono, fontSize: 10, color: T.gold, letterSpacing: "0.1em", marginBottom: 10 }}>ACTIVE DEALS (5)</div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {/* Revenue Chart */}
          <div style={{ background: T.bgCardAlt, border: `1px solid ${T.borderAlt}`, borderRadius: 12, padding: "16px" }}>
            <div style={{ ...mono, fontSize: 10, color: T.gold, letterSpacing: "0.1em", marginBottom: 12 }}>REVENUE CHART</div>
            <div style={{ display: "flex", alignItems: "flex-end", gap: 6, height: 60 }}>
              {REVENUE_BARS.map((h, i) => (
                <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                  ...
                  <span style={{ ...mono, fontSize: 8, color: T.creamDim }}>{REVENUE_MONTHS[i]}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ background: T.bgCardAlt, border: `1px solid ${T.borderAlt}`, borderRadius: 12, padding: "16px" }}>
            <div style={{ ...mono, fontSize: 10, color: T.gold, letterSpacing: "0.1em", marginBottom: 12 }}>NEW OPPORTUNITIES</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {NOTIFICATIONS.map((notif, i) => (
                <div key={i} style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                  <div style={{ width: 8, height: 8, borderRadius: "50%", background: notif.hot ? T.amber : T.gold, flexShrink: 0, marginTop: 4 }} />
                  <div>
                    <div style={{ ...sans, fontSize: 12, fontWeight: 500, color: T.cream, marginBottom: 2 }}>{notif.brand}</div>
                    <div style={{ ...sans, fontSize: 11, color: T.creamDim, lineHeight: 1.4 }}>{notif.message}</div>
                    <div style={{ ...mono, fontSize: 9, color: T.goldDim, marginTop: 3 }}>{notif.time}</div>
                  </div>
                </div>
              ))}
            </div>
...
/* ─────────────────────────────────────────────
   PAGE
───────────────────────────────────────────── */
export default function FeatureManageTalent() {
  return (
    <div style={{ background: T.bg, minHeight: "100vh", color: T.cream, overflowX: "hidden" }}>
      <SEO title="Manage Talent" description="Manage your talent's entire career from one platform. Complete visibility into deals, revenue, and opportunities for managers." />
      <style>{`
        ${fontImport}
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @media (max-width: 768px) {
          ...
          .fp-dashboard-grid { grid-template-columns: 1fr !important; }
      ...
      <section style={{ paddingTop: 160, paddingBottom: 96, textAlign: "center", maxWidth: 860, margin: "0 auto", padding: "160px 24px 96px" }}>
        ...
        <Fade delay={120}>
          <p style={{ ...sans, fontSize: "clamp(16px, 2.2vw, 20px)", color: T.creamDim, lineHeight: 1.7, maxWidth: 640, margin: "0 auto 40px" }}>
            As a manager, you need complete visibility into your talent's deals, revenue, and opportunities. DealStage gives you the command center to manage it all.
          </p>
        </Fade>
        <Fade delay={220}>
        ...
        <Fade delay={280}>
          <div style={{ display: "flex", gap: "1rem", justifyContent: "center", flexWrap: "wrap", marginBottom: "2rem", marginTop: "2rem" }}>
            {[
              { role: "For Brands", text: "Work directly with professional talent managers for faster deal execution" },
        ...
        <Fade delay={320}>
          <div style={{ display: "flex", justifyContent: "center", gap: 24, marginTop: 56, flexWrap: "wrap" }}>
            {[["4x", "More Deals Handled"], ["$340K", "Avg YTD Revenue"], ["Zero", "Missed Opportunities"]].map(([val, lbl], i) => (
      ...
      <section style={{ maxWidth: 1100, margin: "0 auto", padding: "96px 24px" }}>
        <Fade>
          ...
          <h2 style={{ ...serif, fontSize: "clamp(30px, 4vw, 48px)", fontWeight: 700, color: T.cream, marginBottom: 56, maxWidth: 480 }}>
            The command center for talent management
          </h2>
        </Fade>
      ...
      <section style={{ maxWidth: 1100, margin: "0 auto", padding: "96px 24px" }}>
        <Fade>
          ...
          <p style={{ ...sans, fontSize: 16, color: T.creamDim, marginBottom: 52, maxWidth: 520, lineHeight: 1.65 }}>
            Talent profile, active deals, revenue chart, and real-time opportunity alerts — all in one unified view.
          </p>
        </Fade>
        <Fade delay={100}>
      ...
      <section style={{ maxWidth: 860, margin: "0 auto", padding: "96px 24px 120px", textAlign: "center" }}>
        <Fade>
          ...
          <h2 style={{ ...serif, fontSize: "clamp(32px, 5vw, 60px)", fontWeight: 700, color: T.cream, marginBottom: 20 }}>
            Run your roster like a powerhouse agency
          </h2>
          <p style={{ ...sans, fontSize: 17, color: T.creamDim, marginBottom: 40, lineHeight: 1.65, maxWidth: 480, margin: "0 auto 40px" }}>
            Join managers who close 4x more deals, earn higher commissions, and never miss an opportunity.
          </p>
          <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
            <CTAButton primary to="/Onboarding">Start managing talent <ArrowRight size={15} /></CTAButton>
            <CTAButton to="/Demo">Book a demo</CTAButton>
          </div>
        </Fade>
      </section>
