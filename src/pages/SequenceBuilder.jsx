            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  ...
                  className="text-[11px] bg-indigo-100 hover:bg-indigo-200 text-indigo-700 font-mono px-2 py-0.5 rounded transition-colors"
                >
                  {t.token}
                </button>
              </TooltipTrigger>
...
function StepCard({ step, index, total, onChange, onDelete, onGenerate, generating }) {
  ...
  const insertToken = (token) => {
    ...
    onChange(index, { ...step, [field]: current + token });
  };
      ...
      <div className="bg-white border border-slate-200/80 rounded-xl overflow-hidden shadow-sm">
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-slate-700 truncate">{step.subject || `Step ${index + 1} — untitled`}</p>
            <p className="text-xs text-slate-400 flex items-center gap-2">
              <Clock className="w-3 h-3" />
              {index === 0 ? "Sends immediately" : `+${step.delay_days || 0}d delay`}
              <span className="text-slate-300">·</span>
              {step.send_time !== "immediate" && step.send_time ? step.send_time : "any time"}
              <span className="text-slate-300">·</span>
              <span className="capitalize">{step.email_type?.replace(/_/g, " ")}</span>
            </p>
          </div>
          ...
          <div className="border-t border-slate-100 p-4 space-y-3">
            <div className="grid grid-cols-3 gap-3">
                ...
                <Label className="text-xs flex items-center gap-1">
                  ...
                  <TooltipProvider>
                    <Tooltip>
                      ...
                      <TooltipContent className="text-xs">Days after previous step (0 = same day)</TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                </Label>
                <Input
              ...
              <div>
                ...
                <Select
                  value={step.send_time || "immediate"}
                  onValueChange={v => onChange(index, { ...step, send_time: v })}
                >
                  <SelectTrigger className="h-8 text-xs mt-1"><SelectValue /></SelectTrigger>
                  <SelectContent>
            ...
            <div>
              ...
              <Textarea
                ...
                placeholder={`Hi {{first_name}},\n\nI'd love to explore a partnership with {{company}}...\n\nBest,\n{{my_name}}`}
            ...
            >
              {generating === index
                ...
                : <><Sparkles className="w-3 h-3 text-purple-500" /> AI Generate This Step</>}
            </Button>
          </div>
        )}
      </div>
