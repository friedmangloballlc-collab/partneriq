      ...
      <Tabs defaultValue="calendar" className="w-full">
        ...
        <TabsContent value="industries" className="space-y-4 mt-6">
          {industriesLoading ? (
            ...
            <div className="space-y-4">
              {/* Pagination header */}
              <div className="flex items-center justify-between bg-gradient-to-r from-slate-50 to-indigo-50 p-4 rounded-lg border border-slate-200">
                <p className="text-sm font-semibold text-slate-700">
                  {industryGuides.length} Industries • Showing {industryPage * 10 + 1}–{Math.min((industryPage + 1) * 10, industryGuides.length)}
                </p>
                <div className="flex gap-2">
                  ...
                  <span className="text-sm font-medium text-slate-600 px-3 py-1 flex items-center">
                    {industryPage + 1} / {Math.ceil(industryGuides.length / 10)}
                  </span>
                  <Button 
                    size="sm" 
                    variant="outline"
              ...
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-1">
                {industryGuides.slice(industryPage * 10, (industryPage + 1) * 10).map(guide => (
                  <IndustryGuideCard key={guide.id} guide={guide} />
                ))}
              </div>
