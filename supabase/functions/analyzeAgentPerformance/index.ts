...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const prompt = `You are an Agent Performance Analytics AI for a talent agency management platform called Deal Stage.
...
- A step-by-step optimization plan`;
    ...
    return new Response(JSON.stringify({ success: true, analytics: result }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (error) {
