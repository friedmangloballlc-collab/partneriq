...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({ success: true, imported: results, message: `Imported ${results.industryGuides} industry guides and ${results.activationChecklists} checklist items` }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (error) {
