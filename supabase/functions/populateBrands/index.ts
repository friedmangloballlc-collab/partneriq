...
serve(async (req) => {
  ...
  try {
    ...
    const batchSize = body.batch_size || 3; // process 3 industries per call

    // Step 1: Clear existing brands if requested (only on first batch)
    ...
    const hasMore = offset + batchSize < INDUSTRIES.length;

    let totalInserted = 0;
    const errors: string[] = [];

    // Step 2: For each industry in this batch, ask Claude for real brands
...
IMPORTANT: Only include brands that actually exist and are known to work with influencers/creators. No made-up companies.`;
    ...
    return Response.json({
      ...
      has_more: hasMore,
      progress: `${Math.min(offset + batchSize, INDUSTRIES.length)}/${INDUSTRIES.length}`,
