...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!scenario || !scenario.type) {
      return new Response(JSON.stringify({ error: 'Missing scenario or scenario.type' }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }

    const { type, params = {} } = scenario;
    const validTypes = ['add_creator', 'remove_creator', 'change_budget', 'new_campaign', 'market_shift'];
    if (!validTypes.includes(type)) {
      return new Response(JSON.stringify({ error: `Invalid scenario type. Must be one of: ${validTypes.join(', ')}` }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
...
Simulate this scenario thoroughly. Provide realistic projections based on the data. Be specific with numbers and percentages. Consider both positive and negative impacts.`;
    ...
    return new Response(JSON.stringify({ success: true, simulation: result }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (error) {
