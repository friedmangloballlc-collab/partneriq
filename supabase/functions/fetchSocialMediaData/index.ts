...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (apiKey) {
      ...
      // return new Response(JSON.stringify({ success: true, source: 'live_api', platform, action, data: await response.json() }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
