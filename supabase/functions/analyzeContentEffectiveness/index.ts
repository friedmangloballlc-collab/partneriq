...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!emails.length && !sequences.length) {
      return new Response(JSON.stringify({ error: 'No outreach data found to analyze.' }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
...
Return a JSON object:
{
  ...
  "quick_wins": [<array of 3 immediately actionable improvements ranked by expected impact>]
    ...
    return new Response(JSON.stringify({ success: true, analysis: result, totalEmails: emails.length, totalSequences: sequences.length }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (error) {
