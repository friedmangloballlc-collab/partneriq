...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!content || !source_language || !target_languages?.length) {
      return new Response(JSON.stringify({ error: 'Missing required fields: content, source_language, target_languages' }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    if (content_type && !validContentTypes.includes(content_type)) {
      return new Response(JSON.stringify({ error: `Invalid content_type. Must be one of: ${validContentTypes.join(', ')}` }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
...
Ensure translations feel native, not translated. Adapt idioms, humor, references, and formatting conventions (date formats, currency, measurement units) to each locale.`;
    ...
    return new Response(JSON.stringify({ success: true, localization: result }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (error) {
