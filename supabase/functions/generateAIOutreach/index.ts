...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!talent_id || !brand_id || !campaign_type) {
      return new Response(JSON.stringify({ error: 'Missing required fields: talent_id, brand_id, campaign_type' }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }

    const validTypes = ['cold_outreach', 'warm_followup', 're_engagement', 'upsell', 'event_collab'];
    if (!validTypes.includes(campaign_type)) {
      return new Response(JSON.stringify({ error: `Invalid campaign_type. Must be one of: ${validTypes.join(', ')}` }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
...
Also generate A/B variants for the first 2 emails, optimal send times, expected performance metrics, and follow-up triggers that should change the sequence flow.`;
    ...
    return new Response(JSON.stringify({ success: true, outreach: result }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (error) {
