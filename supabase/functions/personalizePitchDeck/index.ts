...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!brand_id || !talent_ids || !Array.isArray(talent_ids) || talent_ids.length === 0) {
      return new Response(JSON.stringify({ error: 'brand_id and talent_ids (array) are required.' }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    if (selectedTalents.length === 0) {
      return new Response(JSON.stringify({ error: 'No matching talents found.' }), { status: 404, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
...
9. Outline 8-12 slides for a professional pitch deck presentation`;
    ...
    return new Response(JSON.stringify({ success: true, pitch_deck: result }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (error) {
